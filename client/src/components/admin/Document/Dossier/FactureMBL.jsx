...
const FactureMBL = () => {
  ...
  const gerateMBL = async (id) => {
    ...
    try {
      ...
      // console.log(resMBL.data);

      const resHBL = await api.get("/hbl/doc/" + id);
      setHblData(resHBL.data);
      // console.log(resHBL.data);

      const resTot = await api.get("/hbl/tot/" + id);
  ...
  const handlePrint = () => {
    ...
    const options = {
      margin: 1,
      filename: "Facture MBL"+mblData.numMBL,
  ...
  return (
    <div>
      <div className="">
        <div className="relative inline-block w-80 m-2">
          ...
          >
            ...
            {mbl.map((v) => (
              <option key={v.idMBL} value={v.idMBL}>
                {v.numMBL}
              </option>
            ))}
          </select>
      ...
      >
        ...
        <p className="font-bold text-lg sm:text-xl">Master Bill of Lading</p>
       {mblData && ( <p className="text-sm sm:text-base">
           {mblData.TransMaritime.paysChargement === "Madagascar"|| "MADAGASCAR" ? "IMPORTATION MARITIME" :"EXPORTATION MARITIME" }
        </p>)}
          ...
          <div className="my-2 ">
            <div className="w-full">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-1">
                ...
                <p className="text-sm sm:text-base">{mblData.numMBL}</p>
