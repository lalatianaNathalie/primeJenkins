...
const AjoutMarchHbl = ({ handleClose, allMarchandiseHBL, isEditMode, selectedPerson }) => {
  const [transMaritimes, setTransMaritimes] = useState([]);
    ...
    const fetchTransMaritimes = async () => {
        try {
    ...
    const handleSelectA = (trans) => {
        if (selectedMaritime && selectedMaritime.idHBLTransaction === trans.idHBLTransaction) {
            setSelectedMaritime(null); // Désélectionne si la même personne est déjà sélectionnée
        } else {
            setSelectedMaritime(trans); // Sélectionner un nouveau transport
            setState(prevState => ({
                ...prevState,
                HBL: trans.numHBL,
    ...
    useEffect(() => {
        const getTransMaritimes = async () => {
            try {
                const data = await fetchTransMaritimes();
                setTransMaritimes(data);
            } catch (err) {
    ...
    const filteredMaritime = transMaritimes.filter(
        (trans) =>
        ...
        {formNo === 1 && (
           <div className="flex flex-col md:flex-row gap-4">
                <div className="w-full md:w-1/3">
                    <InputField label="HBL" name="HBL" value={state.HBL} placeholder="HBL" readOnly />
                        <InfoField label="Client Expediteur : " value={state.clientExp || "------"} />
                        <InfoField label="Client Destinataire : " value={state.clientDest || "------"} />
                        <InfoField label="Date Transaction :" value={state.dateHBLTransaction ? new Date(state.dateHBLTransaction).toISOString().slice(0, 10) : "------"} />
                ...
                <h2 className="text-lg text-center font-bold text-blue-400 mb-4 border-b-2 border-blue-100 pb-2">Transaction HBL disponible</h2>
                <div className="searchContainer flex items-center gap-2">
                        ...
                        />
                        {searchTermT && (
                            <MdClear className="clearIcon cursor-pointer" onClick={() => setSearchTermT("")} />
                        )}
                    </div>
                        ...
                        <table className="table-auto w-full text-left border-collapse">
                            <thead className="text-white bg-blue-200">
                                <tr>
                                    ...
                                    <th className="py-2 px-4 text-left">Date Transaction</th>
                                </tr>
                            </thead>
                            <tbody className="space-y-2">
                                {filteredMaritime.map((trans, index) => (
                                    <tr
                                        key={trans.idHBLTransaction}
                                        onClick={() => handleSelectA(trans)}
                                        className={`hover:bg-blue-200 ${trans === selectedMaritime ? 'bg-blue-500 text-white' : ''} ${index % 2 === 0 ? 'bg-blue-5' : 'bg-blue-50'} ${trans === selectedMaritime ? 'selectedRow' : ''}`}
                                    >
                                        <td>
                                            <input
                                                type="checkbox"
                                                checked={trans === selectedMaritime}
                                                readOnly
                                            />
                                        </td>
